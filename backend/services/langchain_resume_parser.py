...
class LangChainResumeParser:
    ...
    async def build_skill_knowledge_base(self, skill_database: List[Dict]):
        ...
        """
        print(f"📚 [LANGCHAIN] Building knowledge base from {len(skill_database)} skills...")
        ...
        print(f"✅ [LANGCHAIN] Knowledge base built with {len(documents)} skills")
    ...
    async def extract_skills_with_langchain(
        ...
        print(f"🔍 [LANGCHAIN] Retrieved {len(relevant_docs)} relevant skills")
...
{{
    ...
    "job_titles": ["title1", "title2"]
        ...
        try:
            ...
            print(f"✅ [LANGCHAIN] Extracted {len(result.get('matched_skills', []))} skills")
            
            return result
            
        except json.JSONDecodeError as e:
            print(f"⚠️ [LANGCHAIN] JSON parse error: {e}")
    ...
    def save_vector_store(self, path: str = "./skill_vectorstore"):
        ...
        if self.vector_store:
            ...
            print(f"💾 [LANGCHAIN] Vector store saved to {path}")
    ...
    def load_vector_store(self, path: str = "./skill_vectorstore"):
        ...
        try:
            ...
            print(f"📂 [LANGCHAIN] Vector store loaded from {path}")
            return True
        except Exception as e:
            print(f"⚠️ [LANGCHAIN] Could not load vector store: {e}")
            return False
...
class LangChainAdvancedExtractor:
    ...
    async def extract_with_multi_step_chain(
        ...
        print(f"🔑 [STEP 1] Keywords: {keywords[:100]}...")
        ...
        print(f"🔍 [STEP 2] Retrieved: {skill_names}")
