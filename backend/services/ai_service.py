...
class AIService:
            ...
            for resource in module_data.get("resources", []):
                ...
                if "youtube.com" in resource["url"] or "youtu.be" in resource["url"]:
                    try:
                        ...
                        if not is_available:
                            print(f"⚠️  [VALIDATION] YouTube video unavailable: {resource['title']}")
                            ...
                            if alternative:
                                print(f"✅ [VALIDATION] Using alternative: {alternative['url']}")
                                fixed_resources.append(alternative)
                            else:
                                # Keep the original but flag it
                                print(f"⚠️  [VALIDATION] Keeping unavailable video, will warn user")
                            ...
                            fixed_resources.append(resource)
                            print(f"✅ [VALIDATION] YouTube video verified: {resource['title']}")
                    
                    except Exception as e:
                        print(f"⚠️  [VALIDATION] Error validating {resource['title']}: {str(e)}")
    ...
    async def _get_alternative_resource(self, title: str, description: str, module_description: str) -> Dict:
        ...
        try:
            ...
            print(f"Error generating alternative resource: {e}")
            return None
